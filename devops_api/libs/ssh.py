    ...
    def command(self, command):
        ...
        try:
            if self.password:
                ...
                ssh.connect(hostname=self.ip, port=self.port, username=self.username, pkey=key, timeout=5)
...
if __name__ == "__main__":
    ssh = SSH('192.168.201.83', 22, 'root', '123456')
    # result = ssh.command('ls -l')
    # print(result)

    local_file = os.path.join(os.getcwd(), 'custom_pagination.py')
    result = ssh.scp(local_file, 'pagination.py')
    print(result)
